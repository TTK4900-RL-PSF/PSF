...
def RE_RK4(mu):
    return 1 + mu+ 1/2*mu**2+1/6*mu**3 + 1/24*mu**4

def stiffness_ratio(A, B):
    pass
...
def get_terminal_set(sys, t_sys):
    ...
    P, K_scaled = robust_ellipsoid(Ac_set, Bs_set, Hxc, Husrc, hxc, husrc)
    K = K_scaled * B_scale[:, None]
...
def plotEllipsoid(P, Hx, hx, x0=None, savedir=Path(""), name="ellipsoid", x_label="", y_label="", z_label=""):
    if x0 is None:
    ...
    eng.plotEllipsoid(P, Hx, hx, x0, str(Path(savedir, name)), x_label, y_label, z_label)
    eng.quit()
